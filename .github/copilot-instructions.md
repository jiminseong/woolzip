...
## Commit & PR Conventions (from GITHUB_CONVENTION)

- Commit messages: `type(scope): summary` (e.g., `feat(ui): add signal button`).
- PR titles: same format as commits.
- PR description must include:
  - Problem/goal
  - Summary of changes
  - PRD section references (e.g., §10 API)
  - Schema/RLS changes with snippets or links
  - Screenshots/videos for UI or realtime/offline flows
  - Test plan and results
  - Performance impact (bundle size, latency, network roundtrips)
  - Accessibility checks (focus, contrast, screen reader)
  - Security/privacy checks (sensitive data exposure, RLS compliance)

-- Fix ambiguous column references in RPC helpers by qualifying RETURNING targets.
...
begin
  ...
  return query
    insert into public.signals as s (family_id, user_id, type, tag, note, undo_until)
    values (v_family_id, auth.uid(), p_type, p_tag, p_note, v_undo_until)
    returning s.id, s.created_at, s.undo_until;
end;
$$;
...
begin
  ...
  return query
    insert into public.emotions as e (family_id, user_id, emoji, text)
...
begin
  ...
  return query
    insert into public.med_logs as ml (family_id, user_id, medication_id, time_slot)
    values (v_family_id, auth.uid(), p_medication_id, p_time_slot)
    returning ml.id, ml.taken_at, v_med_name as medication_name;
end;
$$;
